...
Notes for 2.1 (vs the 2.0 Mini extension):
    * Hunyuan3D-2.1 needs much more VRAM (~10 GB shape, ~21 GB PBR texture).
    * Shape generation works with just the core dependencies below.
    * PBR texture generation additionally needs the native `custom_rasterizer`
      and `DifferentiableRenderer` extensions, which are compiled lazily from the
      vendored repo source on first texture request (see generator.py).
...
def setup(
    ...
    # Hunyuan3D-2.1 is validated on torch 2.5.1; newer CUDA stacks below
    # are used only where the GPU/CUDA requires them.
    # ------------------------------------------------------------------ #
    if is_mac:
        ...
        print(f"[setup] GPU SM {gpu_sm} -> PyTorch 2.5.1 + CUDA 12.4 (reference build)")
        pip(venv, "install", *torch_pkgs, "--index-url", torch_index)
    else:
        torch_index = "https://download.pytorch.org/whl/cu118"
        torch_pkgs  = ["torch==2.5.1", "torchvision==0.20.1"]
        print(f"[setup] GPU SM {gpu_sm} (legacy) -> PyTorch 2.5.1 + CUDA 11.8")
        pip(venv, "install", *torch_pkgs, "--index-url", torch_index)

    # ------------------------------------------------------------------ #
    # Core dependencies (shape generation)
    # ------------------------------------------------------------------ #
